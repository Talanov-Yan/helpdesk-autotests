...
public class WebActionWebSteps extends AbstractWebSteps {
    ...
    @И("в текущем блоке нажать {string}, если активность кнопки = {boolean}")
    public void clickToToggle(String btnName, Boolean isActive) {
        SelenideElement btnElement = pageManager
                .getCurrentPage()
                .getElement(btnName);
        String attr = btnElement.getAttribute("class");
        if (attr != null) {
            if (isActive == attr.contains("active")) {
                btnElement.click();
                LOGGER.info("В текущем блоке нажата кнопка '{}'", btnName);
            } else {
                LOGGER.info("Условия не выполнены. Кнопка не нажата.");
            }
        } else {
            LOGGER.info("Не найден атрибут Class у элемента '{}'", btnName);
        }
    }

    @И("выбираем {string} по его value = {string}")
    public void useSelectByValue(String name, String value) {
        SelenideElement btnElement = pageManager
                .getCurrentPage()
                .getElement(name);
        btnElement.selectOptionByValue(value);
        LOGGER.info("Выбрали в '{}' значение с value = '{}'",name,value);
    }
}
